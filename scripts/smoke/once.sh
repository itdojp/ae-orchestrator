...
# Usage:
#   GH_REPO=owner/repo AGENT_ROLE=role:IMPL-MED-1 scripts/smoke/once.sh [--mark-done|--no-mark-done] [--issue N] [--timeout SEC] [--sleep SEC]
...
while (($#)); do
  case "$1" in
    ...
    --help|-h) echo "Usage: $0 [--mark-done|--no-mark-done] [--issue <num>] [--timeout <sec>] [--sleep <sec>]"; exit 0;;
...
# Ensure the issue exists and is queued as ready before running watcher once (non-blocking)
pre_flags=()
